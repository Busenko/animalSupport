...
const ANIMATION_EASING = "ease-in-out"; // інші варіанти: linear
                                   // "ease" — повільний старт і кінець
                                   // "ease-in" — повільний старт
                                   // "ease-out" — повільне завершення
                                   // "ease-in-out" — повільний старт і завершення
                                   // "cubic-bezier(x1, y1, x2, y2)" — власна крива
                                   // "step-start" / "step-end" — стрибкоподібна анімація
                                   // "steps(n, start|end)" — розбиває анімацію на n кроків

const ANIMATION_DELAY = 0;

// Напрямок лінії: "closest" (до ближчого краю) або "farthest" (до дальнього краю)
const LINE_DIRECTION = "farthest"; // змінюй на "farthest" для іншого напрямку
...
function drawLines() {
    ...
    for (let i = 0; i < headings.length - 1; i++) {
        ...
        // Визначаємо, з якого краю малювати лінію
        let h1Left;
...
function isHeadingCenteredOrAbove(heading) {
    ...
    // Якщо заголовок перетнув центр або вже вище центру
